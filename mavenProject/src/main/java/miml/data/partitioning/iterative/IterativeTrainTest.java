 ...
 * @version 20201029
 */
public class IterativeTrainTest extends TrainTestBase {
	...
	@Override
	public MultiLabelInstances[] split(double percentageTrain) {
		...
		for (int i = 0; i < folds; i++) {
			try {
				segments[i] = new MultiLabelInstances(singleSegments[i], workingSet.getLabelsMetaData());
	...
	private Instances[] foldsCreation(Instances workingSet, Random random, double[] splitRatio, int numLabels,
		...
		// The vector with the frequencies in the data set (frequency: the
		// number of
		// examples per label)
		...
		// frequencies (first numLabels columns) and of the instances (last
		// column)
		...
		// value [1], when I say value I mean the number of examples for the
		// rarest
		// label.
		...
		for (int lab = 0; lab < numLabels; lab++) {
			...
			// The first element contains the total number of the proper Folds
			// and the
			// rest are the indexes of these folds
			int[] possibleSplits;
			// I share the filtered instances into the splits.
			// The first priority is the splits with the highest desired
			// frequency.
			// The second priority is the split with the highest desired number
			// of
			// instances.
			// If two splits are equivalent for the above two rules I decide
			// randomly
			...
			for (int instancesOfTheLab = 0; instancesOfTheLab < filteredInstancesForLabel
				...
				// I call that function to return the possible folds with the
				// above
				// priorities.
				// possibleSplits[0] contains the total number of possible folds
				// and the
				// rest elements
				// are the indexes of the possible folds.
				possibleSplits = findThePossibleSpit(desiredSplit, smallestFreqLabel[0], numFolds);
				// I decide in which fold to enter the instance. If there are
				// more that
				// one possible folds
		...
		// Special case when I have a number of examples that are not annotated
		// with
	...
	/**
	 * Returns the desired number of examples per label in each fold and in the last
	 * column the total desired number of examples in each fold.
	 *
	...
	private Instances[] takeTheInstancesOfTheLabel(Instances workingSet, int numLabels, int[] labelIndices,
			int[] desiredLabel) {
		// In the returnedInstance in the [0] index is the filtered instances
		// for
		// the desired label
	 ...
	 * sense the label that we will apply the stratification sampling at this point)
	 * and it decides which are the folds that this instance can be inserted. The
	 * first priority is the fold with the smallest number of labels in the desired
	 * label. The second priority is the fold with the less number of instances.
	 *
	 * @param desiredSplit
	 * @param lab
	 * @param numFolds
	...
	private int[] findThePossibleSpit(double[][] desiredSplit, int lab, int numFolds) {
		...
		// Firstly Check which fold has the highest nonnegative value on the
		// label
		// lab
		int maxIndex = 0;
		double maxValue = -1;
		...
		for (int fold = 0; fold < numFolds; fold++) {
			if (desiredSplit[fold][lab] == maxValue) {
				// I will take the split with the maximum number of desired
				// examples
		...
		for (int fold = 0; fold < numFolds; fold++) {
			if (desiredSplit[fold][lab] == maxValue) {
				// I will take as min the fold with the smallest number of
				// instances
				if (desiredSplit[fold][desiredSplit[0].length - 1] == desiredSplit[maxIndex][desiredSplit[0].length
						- 1]) {
					count++;
