...
public class AverageHausdorff extends HausdorffDistance {
	 ...
	 * public double distance(Instances first, Instances second) throws Exception {
	 * 
	 * int nInstancesB1 = first.size(); int nInstancesB2 = second.size();
	 * 
	 ...
	 * 
	 * Instance v = second.instance(j); distances[i][j] = dfun.distance(u, v); } }
	 * 
	 * double sumU = 0.0; for (int i = 0; i < first.size(); ++i) { double
	 * minDistance = Double.MAX_VALUE; for (int j = 0; j < second.size(); ++j) { if
	 * (distances[i][j]<minDistance) minDistance = distances[i][j]; }
	 * sumU+=minDistance; }
	 * 
	 * double sumV = 0.0; for (int j = 0; j < second.size(); ++j) { double
	 * minDistance = Double.MAX_VALUE; for (int i = 0; i < first.size(); ++i) { if
	 * (distances[i][j]<minDistance) minDistance = distances[i][j]; }
	 * sumU+=minDistance; }
	 * 
	 * //System.out.println("Hola: "+(sumU + sumV) / (first.size() +
	 * second.size()));
	 * 
	 * return (sumU + sumV) / (first.size() + second.size()); }
	 */
